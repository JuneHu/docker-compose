...
/**
 * @file
 * Contains \Drupal\cloud_system\RateLimitManager.
 */
...
class RateLimitManager implements RateLimitManagerInterface {
  ...
  public function isEnabled() {
    return (bool) $this->rateLimitingConfig->get('rate_limiter_enable');
  }
  ...
  public function limit(Request $request) {
    ...
    $clientKey = md5(serialize($this->request->headers) . serialize($this->request->getClientIps()));
    switch ($this->rateLimitingConfig->get('rate_limiter_limiting_rule')) {
      case self::RATE_LIMIT_ALL_REQUEST:
        $this->cid = self::RATE_LIMIT_CACHE_PREFIX . 'global_' . $clientKey;
        $this->cacheTag = ['rate_limit_global'];
        break;

      case self::RATE_LIMIT_ON_IP:
        $ip = $request->getClientIp();
        $this->cid = self::RATE_LIMIT_CACHE_PREFIX . str_replace(".", "_", $ip) . '_' . $clientKey;
  ...
  protected function fillBucket() {
    ...
    $this->bucket['bucket_flush_time'] = $this->bucket['bucket_flush_time'] ?? $request_time + (int) $this->rateLimitingConfig->get('rate_limiter_time_cap');

    // If the bucket flush time is lapsed then reset the counter.
    if ($this->bucket['request_time'] > $this->bucket['bucket_flush_time']) {
      $this->bucket['drops'] = 1;
      $this->bucket['bucket_flush_time'] = $request_time + (int) $this->rateLimitingConfig->get('rate_limiter_time_cap');
    }

    // Once the bucket data is stored, check if it's overflowing or not.
    $allowed_drops = $this->rateLimitingConfig->get('rate_limiter_requests');
  ...
  protected function getWhiteListedIps() {
    return array_map('trim', array_filter(explode(PHP_EOL, unserialize($this->rateLimitingConfig->get('rate_limiter_whitelist')))));
  }
  ...
  public function respond() {
    ...
    $custom_message = $this->rateLimitingConfig->get('rate_limiter_message');
    ...
    // Follow https://dev.twitter.com/rest/public/rate-limiting.
    $headers = [
      // The rate limit ceiling for that given request.
      'X-Rate-Limit-Limit' => $this->rateLimitingConfig->get('rate_limiter_requests'),
      // The number of requests left for the 15 minute window.
      'X-Rate-Limit-Remaining' => $this->rateLimitingConfig->get('rate_limiter_time_cap'),
      // The remaining window before the rate limit resets in UTC epoch seconds.
      'X-Rate-Limit-Reset' => $retry
    ];
    // If request header requested for JSON data then respond with JSON.
    if (in_array($type, ['json', 'hal_json'])) {
      return new JsonResponse(['error' => $message], Response::HTTP_TOO_MANY_REQUESTS, $headers);
    }
    else {
