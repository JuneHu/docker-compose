 ...
 */
class CloudSystemEnableRestController implements ContainerInjectionInterface {
        ...
        if ($status == 'disabled') {
          $list[$status]['table']['#rows'][$id]['data']['operations']['data'] = [
            ...
            '#links' => [
              'enable' => [
                'title' => $this->t('Enable'),
                'url' => Url::fromRoute('cloud_system.rest.edit', ['resource_id' => $id]),
              ],
            ],
          ];
        }
          ...
          $list[$status]['table']['#rows'][$id]['data']['operations']['data'] = [
            ...
            '#links' => [
              ...
              'disable' => [
                ...
                'url' => Url::fromRoute('cloud_system.rest.disable', ['resource_id' => $id]),
              ],
              'permissions' => [
   ...
   */
  public function enable($resource_type = '') {
    $resources = [];
    $resources_config = $this->resourceConfigStorage->loadMultiple();

    $enabled_resources = array_combine(array_keys($resources_config), array_keys($resources_config));
    $available_resources = ['enabled' => [], 'disabled' => []];
    $resources_arr = $this->resourcePluginManager->getDefinitions();
    foreach ($resources_arr as $id => $resource) {
      $status = in_array($id, $enabled_resources) ? 'enabled' : 'disabled';
      $available_resources[$status][$id] = $resource;
    }

    $available_disable_resources = $available_resources['disabled'];

    if (!empty($available_disable_resources) && $resource_type) {
      foreach ($available_disable_resources as $id => $disable_resource) {
        $class = $disable_resource['class'];

        // 支持批量全部启用.
        if ($resource_type == 'all') {
          $flag = (FALSE !== strpos($class, 'Plugin\rest\resource')) && (FALSE === strpos($class, 'Entity')) && (FALSE === strpos($class, 'node'));
        }
        else {
          $flag = (FALSE !== strpos($class, $resource_type));
        }

        if ($flag && class_exists($class)) {
          $instance = \Drupal::service('plugin.manager.rest')
            ->createInstance($id);

          if (method_exists($instance, 'get')) {
            $resources[$id][] = 'GET';
            //$resources[$id]['GET']['supported_formats'] = ['json'];
            //$resources[$id]['GET']['supported_auth'] = ['cloud_authentication_provider'];
          }

          if (method_exists($instance, 'post')) {
            $resources[$id][] = 'POST';
            //$resources[$id]['POST']['supported_formats'] = ['json'];
            //$resources[$id]['POST']['supported_auth'] = ['cloud_authentication_provider'];
          }

          if (method_exists($instance, 'put')) {
            $resources[$id][] = 'PUT';
            //$resources[$id]['PUT']['supported_formats'] = ['json'];
            //$resources[$id]['PUT']['supported_auth'] = ['cloud_authentication_provider'];
          }

          if (method_exists($instance, 'delete')) {
            $resources[$id][] = 'DELETE';
            //$resources[$id]['DELETE']['supported_formats'] = ['json'];
            //$resources[$id]['DELETE']['supported_auth'] = ['cloud_authentication_provider'];
          }
      ...
      if (!empty($resources)) {
        foreach ($resources as $resource_id => $methods) {
          ...
          $configuration['authentication'][] = 'cloud_authentication_provider';

          $config->set('granularity', RestResourceConfigInterface::RESOURCE_GRANULARITY);
        ...
        drupal_set_message(t('The resource was enabled successfully.'));
      }
    }

    return new RedirectResponse($this->urlGenerator->generate('cloud_system.rest.list', [], TRUE));
  }
