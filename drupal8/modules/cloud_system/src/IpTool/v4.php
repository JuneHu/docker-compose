...
class v4 extends BaseAddress {
  ...
  public function netClass() {
    if ($this->within(self::$privateNetworks)) {
      ...
      if ($this->within(self::$multicastNetworks)) {
        return 'D';
      }
      else {
        if ($this->mask >= 0xFFFFFF00) {
          return 'C';
        }
        else {
          if ($this->mask >= 0xFFFF0000) {
            return 'B';
          }
          else {
            if ($this->mask >= 0xFF000000) {
              return 'A';
            }
          }
        }
